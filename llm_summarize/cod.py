    ...
    def __call__(self, text_content):
        try:
            ...
            cod_messages = [
                ...
                HumanMessage(
                    content="Here is the input text for you to summarize using the 'Missing_Entities' and 'Denser_Summary' approach:\n\n{}".format(
                        text_content
                    )
                ),
            ]
            ...
            try:
                ...
                print(
                    "[ERROR] The output JSON is not valid of the COD prompt response. LLM Output:\n\n{}\n\n".format(
                        output
                    )
                )
                return
                ...
                print(
                    "[ERROR] The COD output JSON is missing the key `Denser_Summary`. Valid keys are `Missing_Entities` & `Denser_Summary`. LLM Output:\n\n{}\n\n".format(
                        output
                    )
                )
                return
        except Exception as error:
            print(f"[ERROR] Some error happend in COD. Error:\n\n{error}\n\n")
