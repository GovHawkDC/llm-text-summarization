...
from langchain.chains import ReduceDocumentsChain, SequentialChain
...
import llm_summarize.prompts as prompts
...
class MapReduce:
    ...
    def __init__(self, config_dict):
        ...
        kw_extraction_prompt_template = PromptTemplate(
            input_variables=["text_chunk"], template=prompts.KW_EXTRACT_SYSTEM_PROMPT
        )
        kw_extraction_chain = LLMChain(
            llm=self.chat_turbo_llm,
            prompt=kw_extraction_prompt_template,
            output_key="key_words",
        )

        summary_prompt_template = PromptTemplate(
            input_variables=["text_chunk", "key_words"],
            template=prompts.SEQUENCIAL_SUMMARY_PROMPT,
        )
        ...
        try:
            ...
            with get_openai_callback() as openai_cb:
                for idx, doc in enumerate(document_splits, start=1):
                    print(
                        f"[INFO] Map chain extractive summarization for chunk: {idx}/{total_splits}"
                    )
                    extractive_summary_result = self.extractive_summary_chain(
                ...
                final_summary = self.reduce_documents_chain.run(
                    [
                        langchain.schema.document.Document(page_content=chunk)
                        for chunk in summaries_list
                    ]
                )
                ...
                kw_extract_messages = [
                    HumanMessage(
                        content=prompts.KW_EXTRACT_SYSTEM_PROMPT.format(
                            text_chunk=final_summary
                        )
                    )
                ]
            ...
            if (redirect == "cluster_summary") and (
                self.config_dict["cluster_summarization"]["final_dense"] == True
            ):
                do_cod = True
            elif (redirect == "cluster_summary") and (
                self.config_dict["cluster_summarization"]["final_dense"] == False
            ):
                do_cod = False
            elif (redirect is None) and (
                self.config_dict["cluster_summarization"]["final_dense"] == True
            ):
                do_cod = True
            elif (redirect is None) and (
            ...
            print(f"[ERROR] Some error happend in Map Reduce. Error:\n\n{error}\n\n")
